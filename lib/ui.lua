...
function generate_card_ui(_c, full_UI_table, specific_vars, card_type, badges, hide_desc, main_start, main_end, card)
    ...
    if (G.GAME.story_mode or (G.STAGE == G.STAGES.MAIN_MENU and Sagatro.config.DisableOtherJokers) or ((card or {}).displaying_save)) and (card and not card.ability.april_fools_hidden) then
...
function Sagatro.UIDEF.save_description(_id)
  ...
  local stake_key, stake_col = SMODS.stake_from_index(saved_snapshot.GAME.stake), G.C.UI.TEXT_DARK
  if stake_key ~= "stake_white" then
    stake_col = SMODS.Stakes[stake_key].colour or stake_col
  end
