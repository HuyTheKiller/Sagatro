            ...
            if collection_row.cards and type(collection_row.cards) == "table" then
                for _, v in ipairs(collection_row.cards) do
                    ...
                    elseif v.config.center_key == "j_sgt_goldia" and v.config.center.discovered then
                        ...
                        and (G.GAME.story_mode or (G.STAGE == G.STAGES.MAIN_MENU and Sagatro.config.DisableOtherJokers)) and 4 or 3
...
function Card:can_calculate(ignore_debuff, ignore_sliced)
    ...
    if G.GAME.blind then
        if G.GAME.blind.config and G.GAME.blind.config.blind.key == "bl_sgt_knight" and not G.GAME.blind.disabled then
...
function get_blind_amount(ante)
    ...
    if G.GAME.sgt_lenient_score or (G.STAGE == G.STAGES.MAIN_MENU and Sagatro.config.LenientScore) then
...
function Sagatro.resolve_fuel(mod)
    ...
    if card then
        ...
        if card.ability.immutable.states.fuel_left <= 1e-9 then
            ...
            if card.ability.immutable.states.hunger_left > 1e-9 then
